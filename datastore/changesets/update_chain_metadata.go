...
// UpdateChainMetadataChangeset updates existing chain metadata entries in the Datastore.
...
func (UpdateChainMetadataChangeset) VerifyPreconditions(e cldf.Environment, input UpdateChainMetadataChangesetInput) error {
	...
	for _, chainMetadata := range input.ChainMetadata {
		...
		if errors.Is(err, cldfdatastore.ErrChainMetadataNotFound) {
			return fmt.Errorf("chain metadata for chain selector %v does not exist", chainMetadata.ChainSelector)
		}
		if err != nil {
			return fmt.Errorf("failed to retrieve chain metadata for chain selector %v: %w", chainMetadata.ChainSelector, err)
		}
	}
...
// Apply executes the changeset, updating the chain metadata in the Datastore.
func (UpdateChainMetadataChangeset) Apply(
	e cldf.Environment, input UpdateChainMetadataChangesetInput,
) (cldf.ChangesetOutput, error) {
	deps := operations.UpdateChainMetadataDeps{DataStore: e.DataStore}
	opInput := operations.UpdateChainMetadataInput{ChainMetadata: input.ChainMetadata}

	report, err := cldfops.ExecuteOperation(e.OperationsBundle, operations.UpdateChainMetadataOp, deps, opInput)
