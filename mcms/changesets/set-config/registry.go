...
type Registration struct {
	Family string
	// Sequence executes the per-chain set-config operations sequence.
	Sequence *Sequence
...
func Register(reg Registration) {
	if reg.Family == "" {
		panic("mcms set-config: family is required")
	}
	if reg.Sequence == nil {
		panic(fmt.Sprintf("mcms set-config: sequence is required for family %q", reg.Family))
	}
	...
	if _, exists := sequenceRegistry[reg.Family]; exists {
		panic(fmt.Sprintf("mcms set-config: family %q already registered", reg.Family))
	}
...
func registrationForFamily(family string) (Registration, error) {
	...
	if len(registered) == 0 {
		return Registration{}, fmt.Errorf(
			"mcms set-config: no sequence registered for family %q (none registered — blank-import mcms/<family>/set-config or mcms/changesets/set-config/all)",
			family,
		)
	}

	return Registration{}, fmt.Errorf(
		"mcms set-config: no sequence registered for family %q (registered: %s)",
		family,
