...
type Registration struct {
	Family string
	// Sequence executes the per-chain transfer-to-timelock operations sequence.
	Sequence *Sequence
...
// Register adds a family transfer-to-timelock sequence. Panics on invalid input or duplicate registration.
func Register(reg Registration) {
	if reg.Family == "" {
		panic("mcms transfer-to-timelock: family is required")
	}
	if reg.Sequence == nil {
		panic(fmt.Sprintf("mcms transfer-to-timelock: sequence is required for family %q", reg.Family))
	}
	...
	if _, exists := sequenceRegistry[reg.Family]; exists {
		panic(fmt.Sprintf("mcms transfer-to-timelock: family %q already registered", reg.Family))
	}
...
func registrationForFamily(family string) (Registration, error) {
	...
	if len(registered) == 0 {
		return Registration{}, fmt.Errorf(
			"mcms transfer-to-timelock: no sequence registered for family %q (none registered — blank-import mcms/<family>/transfer-to-timelock or mcms/changesets/transfer-to-timelock/all)",
			family,
		)
	}

	return Registration{}, fmt.Errorf(
		"mcms transfer-to-timelock: no sequence registered for family %q (registered: %s)",
		family,
