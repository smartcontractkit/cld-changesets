...
var testFamilyCounter atomic.Uint64
...
func uniqueTestFamily(prefix string) string {
	return fmt.Sprintf("test-transfer-to-timelock-%s-%d", prefix, testFamilyCounter.Add(1))
}
...
func TestRegister_andLookup(t *testing.T) {
	...
	family := uniqueTestFamily("family-a")
...
func TestRegister_validationPanics(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "nil sequence",
			reg:  Registration{Family: uniqueTestFamily("family-b"), Sequence: nil},
		},
	}
...
func TestRegister_duplicatePanics(t *testing.T) {
	...
	family := uniqueTestFamily("family-c")
...
func TestSequenceForFamily_errors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing family", family: uniqueTestFamily("missing")},
	}
...
func TestVerifyForFamily(t *testing.T) {
	...
	nilHookFamily := uniqueTestFamily("family-d")
	Register(Registration{Family: nilHookFamily, Sequence: testSequence("test-seq-d")})

	wrapFamily := uniqueTestFamily("family-e")
	Register(Registration{
		Family:   wrapFamily,
	...
	tests := []struct {
		...
		{
			name:    "missing family",
			family:  uniqueTestFamily("missing-verify"),
