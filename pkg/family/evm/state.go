...
func MaybeLoadMCMSWithTimelockStateDataStoreWithQualifier(env cldf.Environment, chainSelectors []uint64, qualifier string) (map[uint64]*MCMSWithTimelockState, error) {
	if env.DataStore == nil {
		return nil, errors.New("DataStore not available for MCMSWithTimelock state load")
	}
...
func MaybeLoadMCMSWithTimelockChainState(chain cldf_evm.Chain, addresses map[string]cldf.TypeAndVersion) (*MCMSWithTimelockState, error) {
	...
	var (
		...
		multichain    = cldf.NewTypeAndVersion(mcmscontracts.ManyChainMultisig, cldchangesetscommon.Version1_0_0)
		proposerMCMS  = cldf.NewTypeAndVersion(mcmscontracts.ManyChainMultisig, cldchangesetscommon.Version1_0_0)
		bypasserMCMS  = cldf.NewTypeAndVersion(mcmscontracts.ManyChainMultisig, cldchangesetscommon.Version1_0_0)
		cancellerMCMS = cldf.NewTypeAndVersion(mcmscontracts.ManyChainMultisig, cldchangesetscommon.Version1_0_0)
	)
