...
func initAccessController(b operations.Bundle, deps Deps, in InitAccessControllerInput) (InitAccessControllerOutput, error) {
	...
	b.Logger.Infow(
		"initialized access controller",
		"chain", deps.Chain.String(),
		"contract", typeAndVersion.String(),
		"programID", programID,
...
func initializeTimelock(b operations.Bundle, deps Deps, timelockProgram solana.PublicKey,
	...
	var timelockConfig timelockBindings.Config
	err := deps.Chain.GetAccountDataBorshInto(b.GetContext(), familysolana.GetTimelockConfigPDA(timelockProgram, timelockID),
		&timelockConfig)
	if err == nil {
...
func addAccess(b operations.Bundle, deps Deps, in AddAccessInput) (AddAccessOutput, error) {
	...
	timelockConfigPDA := familysolana.GetTimelockConfigPDA(deps.State.TimelockProgram, deps.State.TimelockSeed)
