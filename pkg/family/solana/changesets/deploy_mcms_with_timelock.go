...
func DeployMCMSWithTimelockProgramsSolana(
	...
	chainState, err := familysolana.MaybeLoadMCMSWithTimelockChainState(chain, addresses)
	if err != nil {
	...
	err = initAccessController(e, chainState, mcmscontracts.ProposerAccessControllerAccount, chain, addressBook)
	if err != nil {
		return nil, fmt.Errorf("failed to init proposer access controller: %w", err)
	}
	err = initAccessController(e, chainState, mcmscontracts.ExecutorAccessControllerAccount, chain, addressBook)
	if err != nil {
		return nil, fmt.Errorf("failed to init access controller: %w", err)
	}
	err = initAccessController(e, chainState, mcmscontracts.CancellerAccessControllerAccount, chain, addressBook)
	if err != nil {
		return nil, fmt.Errorf("failed to init access controller: %w", err)
	}
	err = initAccessController(e, chainState, mcmscontracts.BypasserAccessControllerAccount, chain, addressBook)
	if err != nil {
		return nil, fmt.Errorf("failed to init access controller: %w", err)
	}
	...
	err = initMCM(e, chainState, mcmscontracts.BypasserManyChainMultisig, chain, addressBook, &config.Bypasser)
	if err != nil {
		return nil, fmt.Errorf("failed to init bypasser mcm: %w", err)
	}
	err = initMCM(e, chainState, mcmscontracts.CancellerManyChainMultisig, chain, addressBook, &config.Canceller)
	if err != nil {
		return nil, fmt.Errorf("failed to init canceller mcm: %w", err)
	}
	err = initMCM(e, chainState, mcmscontracts.ProposerManyChainMultisig, chain, addressBook, &config.Proposer)
	if err != nil {
...
func DeployMCMSWithTimelockProgramsSolanaV2(
	...
	chain cldf_solana.Chain,
	config cldfproposalutils.MCMSWithTimelockConfig) (*familysolana.MCMSWithTimelockState, error) {
	chainstate, err := familysolana.MaybeLoadMCMSWithTimelockChainStateV2(e.DataStore.Addresses().Filter(datastore.AddressRefByChainSelector(chain.Selector)))
	if err != nil {
		return nil, err
	}
	...
	_, err = operations.ExecuteSequence(e.OperationsBundle, solseqs.DeployMCMSWithTimelockSeq, deps, solseqs.DeployMCMSWithTimelockInput{
		MCMConfig:        config,
