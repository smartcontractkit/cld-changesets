...
func GetMCMSStateFromAddressBook(
	t *testing.T, ab cldf.AddressBook, chain solana.Chain,
) *familysolana.MCMSWithTimelockState {
	t.Helper()

	addresses, err := ab.AddressesForChain(chain.Selector)
	require.NoError(t, err)

	mcmState, err := familysolana.MaybeLoadMCMSWithTimelockChainState(chain, addresses)
	require.NoError(t, err)
