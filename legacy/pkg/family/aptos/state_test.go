...
func TestLoadMCMSAddresses(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		ab := cldf.NewMemoryAddressBook()
		require.NoError(t, ab.Save(chainSel, testAptosMCMSAddr, cldf.NewTypeAndVersion(AptosMCMSType, cldchangesetscommon.Version1_6_0)))
		ds := datastore.NewMemoryDataStore().Seal()
		env := testEnv(t, ab, ds)
	...
	t.Run("no MCMS on chain", func(t *testing.T) {
		...
		ab := cldf.NewMemoryAddressBook()
		other := cldf.NewTypeAndVersion("SomeOtherContract", cldchangesetscommon.Version1_0_0)
	...
	t.Run("wrong version ignored", func(t *testing.T) {
		...
		ab := cldf.NewMemoryAddressBook()
		require.NoError(t, ab.Save(chainSel, testAptosMCMSAddr, cldf.NewTypeAndVersion(AptosMCMSType, cldchangesetscommon.Version1_5_0)))
		ds := datastore.NewMemoryDataStore().Seal()
		env := testEnv(t, ab, ds)
	...
	t.Run("invalid address", func(t *testing.T) {
		...
		ab := cldf.NewMemoryAddressBook()
		require.NoError(t, ab.Save(chainSel, "NotHex", cldf.NewTypeAndVersion(AptosMCMSType, cldchangesetscommon.Version1_6_0)))
		ds := datastore.NewMemoryDataStore().Seal()
		env := testEnv(t, ab, ds)
		...
		require.ErrorContains(t, err, fmt.Sprintf(
			...
			chainSel, AptosMCMSType, cldchangesetscommon.Version1_6_0.String(),
		))
