...
func TestSetConfigMCMSV2EVM(t *testing.T) {
	...
	chain2MCMSState, err := evmstate.MaybeLoadMCMSWithTimelockChainState(chain2, chain2Addrs)
	require.NoError(t, err)
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check new State
			mcmsState, err := evmstate.MaybeLoadMCMSWithTimelockChainState(tt.chain, addrs)
			require.NoError(t, err)
