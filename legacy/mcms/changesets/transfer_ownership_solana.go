...
import (
	...
	cldfutil "github.com/smartcontractkit/cld-changesets/pkg/cldfutil"
...
func (t *TransferToTimelockSolana) VerifyPreconditions(
	...
	for chainSelector, contracts := range config.ContractsByChain {
		err := addressBookContains(envAddressBook(env), chainSelector,
		...
		for _, contract := range contracts {
			exists, err := cldf.AddressBookContains(envAddressBook(env), chainSelector, contract.ProgramID.String())
			if err != nil {
			...
			if (contract.Seed == solstate.PDASeed{}) {
				continue
			}

			exists, err = cldf.AddressBookContains(envAddressBook(env), chainSelector, base58.Encode(contract.Seed[:]))
			if err != nil {
				return fmt.Errorf("failed to search address book for seed (%s): %w", base58.Encode(contract.Seed[:]), err)
			}
			if !exists {
				address := solanaAddress(contract.ProgramID, mcmssolanasdk.PDASeed(contract.Seed))
				exists, err = cldf.AddressBookContains(envAddressBook(env), chainSelector, address)
				if err != nil {
					return fmt.Errorf("failed to search address book for seed (%s): %w", address, err)
				}
			}
			if !exists {
				exists, err = cldf.AddressBookContains(envAddressBook(env), chainSelector, string(contract.Seed[:]))
				if err != nil {
...
func transferOwnershipInstruction(
	programID solana.PublicKey, seed solstate.PDASeed, proposedOwner, ownerPDA, auth solana.PublicKey,
) (solana.Instruction, error) {
	if (seed == solstate.PDASeed{}) {
...
func acceptOwnershipInstruction(programID solana.PublicKey, seed solstate.PDASeed, ownerPDA, auth solana.PublicKey,
) (solana.Instruction, error) {
	if (seed == solstate.PDASeed{}) {
...
func newSeededTransferOwnershipInstruction(
	programID solana.PublicKey, seed solstate.PDASeed, proposedOwner, config, authority solana.PublicKey,
) (solana.Instruction, error) {
...
func newSeededAcceptOwnershipInstruction(
	programID solana.PublicKey, seed solstate.PDASeed, config, authority solana.PublicKey,
) (solana.Instruction, error) {
...
func envAddressBook(env cldf.Environment) cldf.AddressBook {
	return env.ExistingAddresses //nolint:staticcheck // SA1019 until Solana MCMS preconditions use DataStore
}
