...
func addTxsToProposalBatchV2(setConfigTxsChain setConfigTxs, chainSelector uint64, state evmstate.MCMSWithTimelockState) mcmstypes.BatchOperation {
	...
	if setConfigTxsChain.proposerTx != nil {
		result.Transactions = append(result.Transactions,
			evm.NewTransaction(state.ProposerMcm.Address(),
				setConfigTxsChain.proposerTx.Data(), big.NewInt(0), string(mcmscontracts.ProposerManyChainMultisig), nil))
	}
	...
	if setConfigTxsChain.cancellerTx != nil {
		result.Transactions = append(result.Transactions, evm.NewTransaction(state.CancellerMcm.Address(),
			setConfigTxsChain.cancellerTx.Data(), big.NewInt(0), string(mcmscontracts.CancellerManyChainMultisig), nil))
	}
...
func setConfigSolana(
	...
	if cfg.Proposer != nil {
		proposerOps, err := setConfigForRoleSolana(e, chain, *cfg.Proposer, proposerAddress, string(mcmscontracts.ProposerManyChainMultisig), useMCMS, timelockSignerPDA)
		if err != nil {
			return nil, err
		}
	...
	if cfg.Canceller != nil {
		cancellerOps, err := setConfigForRoleSolana(e, chain, *cfg.Canceller, cancellerAddress, string(mcmscontracts.CancellerManyChainMultisig), useMCMS, timelockSignerPDA)
		if err != nil {
			return nil, err
		}
	...
	if cfg.Bypasser != nil {
		bypasserOps, err := setConfigForRoleSolana(e, chain, *cfg.Bypasser, bypasserAddress, string(mcmscontracts.BypasserManyChainMultisig), useMCMS, timelockSignerPDA)
		if err != nil {
			return nil, err
		}
