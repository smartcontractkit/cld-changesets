...
func GrantRolesForTimelock(
...
) (operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput], error) {
	lggr := env.Logger
	ctx := env.GetContext()

	if timelockContracts == nil {
		lggr.Errorw("Timelock contracts not found", "chain", chain.String())
		return operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput]{}, fmt.Errorf("timelock contracts not found for chain %s", chain.String())
	}
	...
	if err != nil {
		return operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput]{}, fmt.Errorf("failed to get admin addresses: %w", err)
	}
	...
	if !isDeployerKeyAdmin && skipIfDeployerKeyNotAdmin {
		...
		return operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput]{}, nil
	}
	if !isDeployerKeyAdmin && !isTimelockAdmin {
		return operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput]{}, errors.New("neither deployer key nor timelock is admin, cannot grant roles")
	}
	...
	if err != nil {
		...
		return operations.SequenceReport[seqevm.SeqGrantRolesTimelockInput, map[uint64][]oputils.EVMCallOutput]{}, err
	}
