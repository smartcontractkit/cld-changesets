...
// CREWorkflowPauseChangeset runs the CRE CLI workflow pause command.
type CREWorkflowPauseChangeset struct{}

// VerifyPreconditions ensures the environment can run CRE and input is valid.
func (CREWorkflowPauseChangeset) VerifyPreconditions(e cldf.Environment, input creops.CREWorkflowPauseInput) error {
	if e.CRERunner == nil {
...
func (CREWorkflowPauseChangeset) Apply(e cldf.Environment, input creops.CREWorkflowPauseInput) (cldf.ChangesetOutput, error) {
	...
	report, err := fwops.ExecuteOperation(e.OperationsBundle, creops.CREWorkflowPauseOp, deps, input)
